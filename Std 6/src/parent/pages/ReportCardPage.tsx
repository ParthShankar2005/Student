...
function loadStats(): UserStats {
  ...
  return {
    ...
    streak: 3,
    badges: [
      { id: 'first-login', name: 'First Login', icon: 'STAR', description: 'Logged in for the first time' },
      { id: 'streak-3', name: 'Hat Trick', icon: 'FIRE', description: '3-day streak' },
    ],
    attendance: [],
...
function loadHomework(): HomeworkItem[] {
  try {
    ...
    if (raw) return JSON.parse(raw) as HomeworkItem[];
  } catch {
    // ignore localStorage parse issues
  }
...
function loadAttendance(): AttendanceSnapshot {
  try {
    ...
    if (raw) return JSON.parse(raw) as AttendanceSnapshot;
  } catch {
    // ignore localStorage parse issues
  }
