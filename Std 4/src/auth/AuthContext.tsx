...
const SESSION_STORAGE_KEY = 'ssms_std4_student_session_v1';
const LEGACY_ROLE_STORAGE_KEY = 'ssms_std4_auth_role_legacy';
const DEFAULT_USER: AuthUser = { role: 'student', grade: 4, name: 'Explorer' };
...
function persistSession(studentId: string): void {
  try { localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify({ studentId })); } catch {}
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const switchToStudentView = useCallback(() => {
    setAuthState(prev => prev.user.role === 'student' && !prev.parentVerified ? prev : ({ ...prev, parentVerified: false, user: { ...prev.user, role: 'student' } }));
  ...
  const switchRole = useCallback((): boolean => {
    if (!authState.isAuthenticated) return false;
    if (authState.user.role === 'parent') { switchToStudentView(); return true; }
  ...
  const setGrade = useCallback((grade: number) => {
    setAuthState(prev => ({ ...prev, user: { ...prev.user, grade }, studentProfile: prev.studentProfile ? { ...prev.studentProfile, grade } : prev.studentProfile }));
  }, []);
